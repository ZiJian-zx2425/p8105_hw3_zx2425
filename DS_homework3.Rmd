...
nynoaadat=ny_noaa
```
Firstly, the dataframe contains 2595176 objects and 7 variables. Variables include  `r colnames(ny_noaa)` 
...
nyno_fre
```
First, we multiply 10 to each variable of 'snow' and 'snwd', this is because we do not want to lose data precision compared with the solution that is to divide by 10. At the same time, after viewing the frequency table of snow we can see the value of 0 is significantly larger than other values, This possibly because the possible of the snow of the city is low. Besides, we are comparing the frequency of no snow with the frequency of snow at a certain level at the city.
